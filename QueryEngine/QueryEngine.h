...
namespace Query
{
	struct PageResult
	{
		...
		double score;
		friend bool operator<(const PageResult& l, const PageResult& r) { return l.p.size < r.p.size; };
	};
	...
	struct ListPointer
	{
		...
		friend bool operator<(const ListPointer& l, const ListPointer& r) { return l.num_docs < r.num_docs; };
	};
